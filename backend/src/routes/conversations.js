...
router.post('/:id/enviar-cartera', async (req, res) => {
  ...
  const tenant = await Tenant.findById(conversation.tenantId).lean()
  if (!tenant) return res.status(404).json({ error: 'Línea no encontrada' })

  try {
    const io = req.app.get('io')
    const { message, cartera } = await enviarRecordatorioCartera(tenant, conversation, customer, 'manual', io)
